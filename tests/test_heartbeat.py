...
def test_send_heartbeat_posts_correct_payload():
    ...
    with patch("vardrrunner.commands.heartbeat.config.require_auth", return_value=("http://api", "key")), \
...
def test_send_heartbeat_skips_when_not_authenticated():
    import typer
    with patch("vardrrunner.commands.heartbeat.config.require_auth",
