...
def test_run_subfinder_command_extracts_wildcards(tmp_path):
    ...
    with patch("vardrrunner.commands.run.config.require_auth", return_value=("http://api", "key")), \
...
def test_list_jobs_shows_table(capsys):
    client = MagicMock()
    client.pending_jobs.return_value = [
        {"id": "abc123", "tool_type": "httpx", "target_source": "scope",
         "config": {}, "created_at": "2026-06-09T10:00:00"},
    ]
